...
void sendGadgetPassed(int gadget_index) // Проверяем прошел ли игрок какой-нибудь гаджет или применяем скипы оператора
{
  ...
  Serial1.write(0xAD);
  delay(15);
  Serial.println("GADGET_SEND_DONE: " + String(gadget_index));
  lcd.setCursor(0, 1);
  lcd.print("GSend:");
...
void sendGadgetUndone(int gadget_index) // Проверяем прошел ли игрок какой-нибудь гаджет или применяем скипы оператора
{
  digitalWrite(RSTXCNTRL, HIGH);  // Init Transmitter
  digitalWrite(REPIN, HIGH);
  delay(10);
  Serial1.write(0xAD);
  delay(15);
  Serial.println("GADGET_SEND_UNDONE: " + String(gadget_index));
  for (int s = 0; s < GCOUNT; s++)
  {
    if (s == gadget_index) Serial1.write(0x05);
    delay(7);  //was 7
  }
  Serial1.write(0xFF);
  delay(25);  //was 25 before , later was 10 till feb 25, 
...
void sendGadgetState(int gadget_index, int state) // Проверяем прошел ли игрок какой-нибудь гаджет или применяем скипы оператора
{
  ...
  Serial1.write(0xAE);
  delay(5);
  byte payload = (gadget_index & 0b00011111) | (state << 5);
  Serial1.write(0xFF & payload);
  Serial1.write(0xFF);
  delay(5);
  digitalWrite(RSTXCNTRL, LOW);  // Stop Transmitter
  digitalWrite(REPIN, LOW);
